<?php
/**
 * Workout Plans - Delete Plan
 * Level Up Fitness - Gym Management System
 */

require_once dirname(dirname(dirname(__FILE__))) . '/includes/header.php';

requireLogin();

$planId = sanitize($_GET['id'] ?? '');

if (empty($planId)) {
    setMessage('Invalid plan ID', 'error');
    redirect(APP_URL . 'modules/workouts/');
}

try {
    // Get plan info first
    $stmt = $pdo->prepare("SELECT * FROM workout_plans WHERE workout_plan_id = ?");
    $stmt->execute([$planId]);
    $plan = $stmt->fetch();

    if (!$plan) {
        setMessage('Plan not found', 'error');
        redirect(APP_URL . 'modules/workouts/');
    }

    // If confirmed
    if (isset($_GET['confirm']) && $_GET['confirm'] == 'yes') {
        // Delete plan
        $deleteStmt = $pdo->prepare("DELETE FROM workout_plans WHERE workout_plan_id = ?");
        $deleteStmt->execute([$planId]);

        logAction($_SESSION['user_id'], 'DELETE_PLAN', 'Workouts', 'Deleted plan: ' . $plan['plan_name']);

        setMessage('Workout plan deleted successfully', 'success');
        redirect(APP_URL . 'modules/workouts/');
    }

    // If cancel
    if (isset($_GET['confirm']) && $_GET['confirm'] == 'no') {
        redirect(APP_URL . 'modules/workouts/view.php?id=' . $planId);
    }

} catch (Exception $e) {
    setMessage('Error: ' . $e->getMessage(), 'error');
    redirect(APP_URL . 'modules/workouts/');
}
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Confirm Delete - <?php echo APP_NAME; ?></title>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.3.0/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
</head>
<body>
    <div class="container mt-5">
        <div class="row justify-content-center">
            <div class="col-md-6">
                <div class="card border-danger">
                    <div class="card-header bg-danger text-white">
                        <h5 class="mb-0"><i class="fas fa-exclamation-triangle"></i> Confirm Deletion</h5>
                    </div>
                    <div class="card-body">
                        <p class="lead">Are you sure you want to delete this workout plan?</p>
                        
                        <div class="alert alert-danger">
                            <strong>Plan:</strong> <?php echo htmlspecialchars($plan['plan_name']); ?><br>
                            <strong>Plan ID:</strong> <?php echo htmlspecialchars($planId); ?><br>
                            <strong>Member:</strong> <?php echo htmlspecialchars($plan['member_id']); ?><br>
                            <strong>Goal:</strong> <?php echo htmlspecialchars($plan['goal']); ?>
                        </div>

                        <p class="text-muted">
                            <i class="fas fa-info-circle"></i> 
                            This action cannot be undone.
                        </p>

                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            <a href="<?php echo APP_URL; ?>modules/workouts/view.php?id=<?php echo $planId; ?>" class="btn btn-outline-secondary">
                                <i class="fas fa-times"></i> Cancel
                            </a>
                            <a href="?id=<?php echo urlencode($planId); ?>&confirm=yes" class="btn btn-danger">
                                <i class="fas fa-trash"></i> Delete Plan
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.3.0/js/bootstrap.bundle.min.js"></script>
</body>
</html>
